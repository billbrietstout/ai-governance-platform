  ...
  const payload = {
    ...
    attachments: [{
      ...
      blocks: [
        {
          type: 'header',
          text: { type: 'plain_text', text: `${config.emoji} ${config.label} — ${alert.title}`, emoji: true },
        },
        {
          type: 'section',
          fields: [
            ...
            ...(alert.systemName ? [{ type: 'mrkdwn', text: `*AI System:*\n${alert.systemName}` }] : []),
            ...(alert.layer ? [{ type: 'mrkdwn', text: `*CoSAI SRF Layer:*\n${alert.layer}` }] : []),
          ],
        },
        {
          type: 'section',
...
async function generateAlertsForOrg(org) {
  ...
  const unownedSystems = await prisma.aISystem.findMany({
    where: {
  ...
  const recentScoreDrop = await prisma.complianceScore.findFirst({
    where: {
      organizationId: org.id,
      dropAmount: { gte: 10 },
  ...
  const upcomingDeadlines = await prisma.regulatoryDeadline.findMany({
    where: {
      organizationId: org.id,
      dueDate: {
        gte: new Date(),
...
async function main() {
  console.log(`[daily-alerts] Starting ${DRY_RUN ? '(DRY RUN) ' : ''}— ${new Date().toISOString()}`)
  ...
  console.log(`[daily-alerts] Complete — emails: ${totalEmailsSent}, slack: ${totalSlackSent}, skipped: ${totalSkipped}`)
}

main()
  .catch(err => { console.error('[daily-alerts] Fatal:', err); process.exit(1) })

...
export function SessionExpiryWarning() {
  ...
  const checkExpiry = useCallback(async () => {
    try {
      ...
      if (msLeft <= 0) { router.push("/login"); return; }
  ...
  async function handleExtend() {
    setExtending(true);
    try { await fetch("/api/auth/session", { cache: "no-store" }); setShowWarning(false); router.refresh(); }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="mx-4 w-full max-w-sm rounded-2xl bg-white shadow-xl border border-gray-100 p-6">
        ...
        <div className="space-y-2">
          ...
          <button onClick={() => router.push("/api/auth/signout")} className="w-full py-2.5 px-4 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-200 transition-colors">
            Sign out now
          </button>
        </div>
        <p className="mt-4 text-center text-xs text-gray-400">AI Readiness Platform keeps you signed in for 8 hours of activity.</p>
      </div>
    </div>
  );
}
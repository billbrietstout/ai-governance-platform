...
export default async function VendorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-4xl flex-col gap-6 px-6 py-10">
      <div>
        <Link href="/layer5-supply-chain/vendors" className="text-sm text-navy-400 hover:underline">
          ← Vendor Registry
        </Link>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight">{v.vendorName}</h1>
        <div className="mt-2 flex items-center gap-4">
          <VendorAssuranceScore total={v.assuranceScore.total} breakdown={v.assuranceScore.breakdown} />
          <span className="text-sm text-slatePro-400">{v.vendorType ?? "—"}</span>
        </div>
      </div>

      <section>
        <h2 className="mb-2 text-lg font-medium">Assurance Breakdown</h2>
        <div className="grid gap-2 sm:grid-cols-2">
          {Object.entries(v.assuranceScore.breakdown).map(([key, val]) => (
            <div
              key={key}
              className="flex justify-between rounded border border-slatePro-700 bg-slatePro-900/30 px-3 py-2"
            >
              <span className="text-slatePro-400">{key.replace(/([A-Z])/g, " $1").trim()}</span>
              <span className="font-medium text-slatePro-200">{Math.round(val * 100)}%</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {v.expiredEvidence.length > 0 && (
        <section>
          <h2 className="mb-2 text-lg font-medium text-amber-400">Expired Evidence</h2>
          <ul className="space-y-1 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3">
            {v.expiredEvidence.map((e, i) => (
              <li key={i} className="text-sm">
                ...
                {e.expiredAt && (
                  <span className="ml-1 text-slatePro-500">
      ...
      <section>
        <h2 className="mb-2 text-lg font-medium">Evidence & Certifications</h2>
        <div className="space-y-2 rounded-lg border border-slatePro-700 bg-slatePro-900/30 p-4">
      ...
      {v.evidenceLinks && typeof v.evidenceLinks === "object" && Array.isArray(v.evidenceLinks) && (
        <section>
          <h2 className="mb-2 text-lg font-medium">Evidence Links</h2>
      ...
      <section>
        <h2 className="mb-2 text-lg font-medium">Contract Checklist</h2>
...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between">
      <span className="text-slatePro-400">{label}</span>
      <span className="text-slatePro-200">{value}</span>
    </div>
  );
}

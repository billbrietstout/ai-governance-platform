  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-slate-900">Drift Detection</h1>
        <p className="mt-1 text-slate-600">
          Monitor AI assets for governance drift — stale scans, policy failures, and rising findings.
        </p>
      </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-4 py-3">
            <h3 className="text-sm font-semibold text-slate-700">
              Flagged Assets <span className="ml-1 rounded bg-red-100 px-1.5 py-0.5 text-xs text-red-700">{flagged.length}</span>
            </h3>
          </div>
          <div className="divide-y divide-slate-50">
            {flagged.map(d => (
              <div key={d.assetId} className="flex flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {d.euRiskLevel === "HIGH" && (
                      <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs text-red-700">HIGH RISK</span>
                    )}
                  </div>
                  <div className="mt-1 flex flex-wrap gap-1">
                    {d.signals.map(signal => {
                      const meta = SIGNAL_LABELS[signal] ?? { label: signal, color: "bg-slate-100 text-slate-600" };
                      return (
                        <span key={signal} className={`rounded px-1.5 py-0.5 text-xs ${meta.color}`}>
                          {meta.label}
                        </span>
                      );
                    })}
      ...
      {clean.length > 0 && (
        <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-100 px-4 py-3">
            <h3 className="text-sm font-semibold text-slate-700">
              Clean Assets <span className="ml-1 rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-700">{clean.length}</span>
            </h3>
          </div>
          <div className="divide-y divide-slate-50">
            {clean.map(d => (
              <div key={d.assetId} className="flex items-center justify-between px-4 py-2.5">
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500">
                  {d.daysSinceScan !== null && <span>{d.daysSinceScan}d since scan</span>}
                  <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-emerald-700">Clean</span>
                </div>
              </div>
            ))}
          </div>

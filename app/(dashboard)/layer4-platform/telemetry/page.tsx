...
const SCAN_TYPE_LABELS: Record<string, string> = {
  SBOM: "SBOM", SBOM_DEPENDENCY: "SBOM Deps", VULN: "Vulnerability",
  SECRETS: "Secrets", POLICY: "Policy", LICENSE: "License",
  MODEL_SCAN: "Model Scan", DATASET_PII: "Dataset PII", RED_TEAM: "Red Team"
};
...
function PolicyBadge({ passed }: { passed: boolean | null }) {
  ...
  return passed
    ? <span className="rounded bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700">PASS</span>
  ...
  return (
    <main className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10">
      <div>
        ...
        <p className="mt-1 text-slate-600">Scan activity, coverage, and findings across all AI assets.</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          { label: "Critical Findings", value: data.summary.criticalFindings, alert: data.summary.criticalFindings > 0 },
          { label: "Failed Policies", value: data.summary.failedPolicies, alert: data.summary.failedPolicies > 0 },
          { label: "Coverage", value: `${data.summary.coveragePercent}%` },
          { label: "Unscanned Assets", value: data.summary.unscannedCount, alert: data.summary.unscannedCount > 0 }
        ].map(card => (
          <div key={card.label} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p className="text-sm font-medium text-slate-600">{card.label}</p>
            <p className={`mt-1 text-2xl font-bold ${card.alert ? "text-red-600" : "text-slate-900"}`}>
              {card.value}
            </p>
          </div>
        ))}
              ...
              return (
                <div key={type} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 rounded-full bg-slate-100">
                    <div className="h-2 rounded-full bg-navy-500" style={{ width: `${pct}%` }} />
                  </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {data.recentActivity.map(scan => (
                <tr key={scan.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-2 text-slate-600">{SCAN_TYPE_LABELS[scan.scanType] ?? scan.scanType}</td>
                  <td className="px-4 py-2 text-right text-slate-700">{scan.findingsCount}</td>
                  <td className={`px-4 py-2 text-right font-medium ${(scan.criticalFindings ?? 0) > 0 ? "text-red-600" : "text-slate-400"}`}>
                    {scan.criticalFindings ?? 0}
                  </td>
                  <td className="px-4 py-2"><PolicyBadge passed={scan.policyPassed ?? null} /></td>

...
function CardSkeleton({ height = "h-48" }: { height?: string }) {
  return (
    <div className={`rounded-lg border border-slate-200 bg-white p-4 shadow-sm animate-pulse ${height}`}>
...
async function KpiCards({ orgId }: { orgId: string }) {
  ...
  return (
    <div className="grid gap-3 overflow-visible sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-8">
      <KpiCard label="Total AI Assets" value={kpis.totalAssets} href="/layer3-application/assets" icon={<Bot className="h-4 w-4 text-blue-600" />} delta={deltas.totalAssetsDelta} />
      <KpiCard label="Compliance Score" value={`${kpis.complianceScore}%`} icon={<ShieldCheck className={`h-4 w-4 ${kpis.complianceScore >= 70 ? "text-emerald-600" : kpis.complianceScore >= 30 ? "text-amber-600" : "text-red-600"}`} />} />
      <KpiCard label="Critical Risks" value={kpis.criticalRisks} href="/layer3-application/assets" icon={<AlertOctagon className="h-4 w-4 text-red-600" />} delta={deltas.criticalRisksDelta} />
      <KpiCard label="EU High-Risk" value={kpis.euHighRisk} icon={<Scale className="h-4 w-4 text-amber-600" />} tooltip="EU AI Act: classification of AI systems requiring conformity assessment" />
      <KpiCard label="No Accountability" value={kpis.withoutAccountability} href="/layer3-application/accountability" icon={<UserX className="h-4 w-4 text-red-600" />} />
      <KpiCard label="Stale Cards (>30d)" value={kpis.staleCards} href="/layer5-supply-chain/cards" icon={<Clock className="h-4 w-4 text-amber-600" />} />
      <KpiCard label="Failed Scan Policies" value={kpis.failedScans} href="/layer5-supply-chain/scanning" icon={<XCircle className="h-4 w-4 text-red-600" />} />
      <KpiCard label="Vendors Expiring" value={kpis.vendorsExpiring} href="/layer5-supply-chain/vendors" icon={<Building className="h-4 w-4 text-amber-600" />} delta={deltas.vendorsExpiringDelta} />
    </div>
  );
}
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <LayerSankeyDiagram
        layerData={sankeyRes.data.nodes.map((n, i) => {
          const layerKey = ["LAYER_1_BUSINESS","LAYER_2_INFORMATION","LAYER_3_APPLICATION","LAYER_4_PLATFORM","LAYER_5_SUPPLY_CHAIN"][i];
        ...
        layerLinks={{ L1: "/layer1-business", L2: "/layer2-information", L3: "/layer3-application/assets", L4: "/layer4-platform", L5: "/layer5-supply-chain" }}
      />
      <div className="mt-3 flex flex-wrap gap-2">
        {layerRes.data.map((l) => {
          const label = { LAYER_1_BUSINESS:"L1", LAYER_2_INFORMATION:"L2", LAYER_3_APPLICATION:"L3", LAYER_4_PLATFORM:"L4", LAYER_5_SUPPLY_CHAIN:"L5" }[l.layer] ?? l.layer;
          const href = LAYER_LINKS[l.layer];
          return (
            <Link key={l.layer} href={href} className="flex items-center gap-1.5 rounded border border-slate-200 bg-slate-50 px-3 py-1.5 text-sm transition hover:border-slate-300">
              <span className="font-medium text-slate-700">{label}</span>
              {l.riskCount > 0 ? <span className="rounded bg-red-100 px-2 py-0.5 text-xs text-red-700">{l.riskCount} risks</span> : <span className="rounded bg-slate-100 px-2 py-0.5 text-xs text-slate-500">0 risks</span>}
    ...
    <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {!maturityRes.data.lastAssessedAt ? (
        <div className="flex items-center justify-between rounded border border-amber-200 bg-amber-50/50 px-4 py-3">
          ...
          <Link href="/maturity" className="rounded bg-navy-600 px-4 py-2 text-sm font-medium text-white hover:bg-navy-500">Complete Assessment</Link>
        </div>
      ) : (
        <>
          <div className="mb-3 flex items-center justify-between">
            <p className="text-sm text-slate-600" suppressHydrationWarning>
              Last assessed {Math.floor((Date.now() - new Date(maturityRes.data.lastAssessedAt).getTime()) / (24 * 60 * 60 * 1000))} days ago
            </p>
            <Link href="/maturity" className="text-sm font-medium text-navy-600 hover:underline">Retake →</Link>
          </div>
          <div className="flex justify-center">
            <MaturityRadarChart scores={maturityRes.data.scores as LayerScores} targetLevel={Math.min(maturityRes.data.maturityLevel + 1, 5)} size={280} interactive={false} />
          </div>
          <Link href="/maturity" className="mt-3 block text-center text-sm font-medium text-navy-600 hover:underline">View full assessment →</Link>
        </>
      )}
    </div>
  );
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <p className="text-2xl font-semibold text-gray-900">{cascadeRes.data.met} / {cascadeRes.data.totalRequirements} met</p>
            <div className="flex-1">
              <div className="h-2 overflow-hidden rounded-full bg-slate-200">
                <div className="h-full rounded-full bg-navy-500 transition-all" style={{ width: `${cascadeRes.data.pct}%` }} />
              </div>
            </div>
          </div>
          <Link href="/layer1-business/regulatory-cascade" className="mt-2 text-sm text-navy-600 hover:underline">View cascade →</Link>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            {gapsRes.data.length === 0 ? (
              ...
              gapsRes.data.map((g) => (
                <li key={`${g.assetId}-${g.controlId}`} className="flex items-center justify-between gap-2 rounded border border-slate-200 bg-slate-50 px-3 py-2">
                  <div>
                    <Link href={`/layer3-application/assets/${g.assetId}`} className="font-medium text-navy-600 hover:underline">{g.assetName}</Link>
                  ...
                  <Link href={`/layer3-application/assets/${g.assetId}`} className="shrink-0 text-xs text-navy-600 hover:underline">View →</Link>
                </li>
              ))
            )}
          </ul>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mb-2 flex gap-4 text-xs text-slate-600">
            ...
            <span className={vendorRes.data.filter((v) => v.expiredCount > 0).length > 0 ? "text-amber-600" : ""}>{vendorRes.data.filter((v) => v.expiredCount > 0).length} with expired evidence</span>
          </div>
          <div className="space-y-2">
            {vendorRes.data.length === 0 ? <p className="text-sm text-gray-500">No vendors</p> : vendorRes.data.slice(0, 3).map((v) => (
              <div key={v.id} className="flex items-center justify-between rounded border border-slate-200 bg-slate-50 px-3 py-2">
                <Link href={`/layer5-supply-chain/vendors/${v.id}`} className="font-medium text-navy-600 hover:underline">{v.name}</Link>
                <span className={`text-sm ${v.expiredCount > 0 ? "text-amber-600" : "text-emerald-600"}`}>{Math.round(v.score * 100)}% {v.expiredCount > 0 && "⚠"}</span>
              </div>
            ))}
          </div>
          <Link href="/layer5-supply-chain/vendors" className="mt-2 text-sm text-navy-600 hover:underline">View all vendors →</Link>
        </div>
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
...
export default async function CommandCenterPage({
  ...
  const user = session?.user as { id?: string; role?: string; email?: string | null; orgId?: string } | undefined;
  ...
  const nextSteps = maturityForWelcome.data.nextSteps as { layer: string; action: string; priority: string }[];
  ...
  if (role === "CAIO" || role === "ADMIN") {
    try { penaltyRes = await caller.dashboard.getEUPenaltyExposure(); } catch {}
  }
    ...
    <main className="flex flex-col gap-6">
          ...
          <ul className="mt-3 space-y-2">
            {(personaLabel ? topNextSteps.slice(1) : topNextSteps).map((s, i) => (
              <li key={`${s.layer}-${s.action}`} className="flex items-center gap-2 text-sm text-navy-700">
                <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-navy-200 text-xs font-medium text-navy-800">{i + 1}</span>
                {s.action}
              </li>
            ))}
          </ul>
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-600">Your AI readiness posture across the CoSAI five-layer framework</p>
        </div>
      </div>
        ...
        <div className="overflow-visible rounded-lg border border-slate-200 border-l-4 border-l-red-600 bg-white shadow-sm">
          <div className="bg-red-50/50 p-4">
            <div className="flex items-center gap-2">
              ...
              <Tooltip content="Range based on high-risk asset count and Article 99 penalty tiers." side="bottom">
            ...
            <Link href="/reports/gap-analysis" className="mt-2 inline-block text-sm font-medium text-red-500 hover:text-red-600 hover:underline">View breakdown →</Link>
          </div>
        </div>
      )}
...
function KpiCard({ label, value, href, icon, delta, tooltip }: {
  label: string; value: string | number; href?: string; icon?: React.ReactNode; delta?: string | null; tooltip?: string;
}) {
  const content = (
    <div className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm transition hover:border-slate-300 hover:shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1">
          ...
          {tooltip && <Tooltip content={tooltip}><Info className="h-3 w-3 text-slate-400" /></Tooltip>}
        </div>
        {icon}
      </div>

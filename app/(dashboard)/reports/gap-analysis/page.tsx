    ...
    <main className="mx-auto flex min-h-dvh max-w-6xl flex-col gap-6 px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold tracking-tight">Gap Analysis with Remediation Roadmap</h1>
        </div>
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-slatePro-700 bg-slatePro-900/50">
              ...
              <th className="px-4 py-2 text-left font-medium text-slatePro-300">Control</th>
              <th className="px-4 py-2 text-left font-medium text-slatePro-300">Layer</th>
              <th className="px-4 py-2 text-left font-medium text-slatePro-300">Remediation</th>
            </tr>
          </thead>
          <tbody>
            {gaps.length === 0 ? (
              ...
              gaps.map((g) => (
                <tr key={`${g.assetId}-${g.controlId}`} className="border-b border-slatePro-800">
                  <td className="px-4 py-2">
                    <Link href={`/layer3-application/assets/${g.assetId}`} className="text-navy-400 hover:underline">
                      {g.assetName}
                    </Link>
                  </td>
                  <td className="px-4 py-2 text-slatePro-200">{g.controlId}: {g.title}</td>
                  <td className="px-4 py-2 text-slatePro-200">{g.cosaiLayer ?? "—"}</td>
                  <td className="px-4 py-2 text-slatePro-400">Owner TBD · Due date TBD</td>
                </tr>
              ))
            )}
          </tbody>

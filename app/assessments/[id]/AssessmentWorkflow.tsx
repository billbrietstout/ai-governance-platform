...
export function AssessmentWorkflow({ assessmentId, assetId, layers, byLayer, status }: Props) {
  ...
  const attested = controls.filter((c) => c.attestation?.status === "COMPLIANT" || c.attestation?.status === "NOT_APPLICABLE");
    ...
    <div className="flex gap-4">
      <div className="w-48 shrink-0 space-y-1">
        {layers.map((layer) => {
          ...
          const a = ctrls.filter((c) => c.attestation?.status === "COMPLIANT" || c.attestation?.status === "NOT_APPLICABLE").length;
          ...
          return (
            <button
              ...
              className={`w-full rounded px-2 py-1.5 text-left text-sm ${
                activeLayer === layer ? "bg-navy-600/30 text-navy-300" : "text-slatePro-400 hover:bg-slatePro-800"
              }`}
            >
              <span className="block truncate">{layer.replace("LAYER_", "L").replace(/_/g, " ")}</span>
      ...
      <div className="flex-1 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-slatePro-300">{activeLayer} — {attested.length}/{controls.length} attested</span>
        </div>
        ...
        <div className="space-y-3">
          {controls.map((c) => (
            <div
              key={c.id}
              className="rounded-lg border border-slatePro-700 bg-slatePro-900/30 p-4"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="font-medium text-slatePro-200">{c.controlId}: {c.title}</div>
                  <div className="text-xs text-slatePro-500">{c.framework.code}</div>
                  {c.description && (
                    <p className="mt-1 text-sm text-slatePro-400">{c.description}</p>
                  )}
                  {c.implementationGuidance && (
                    <p className="mt-1 text-xs text-slatePro-500">{c.implementationGuidance}</p>
                  )}
                </div>
                {status !== "APPROVED" && (

...
const THIRTY_DAYS_AGO = (() => { const d = new Date(); d.setDate(d.getDate() - 30); return d; })();
const NINETY_DAYS_AGO = (() => { const d = new Date(); d.setDate(d.getDate() - 90); return d; })();
...
export const layer4Router = createTRPCRouter({
  ...
  getTelemetry: protectedProcedure.query(async ({ ctx }) => {
    ...
    const byType = allScans.reduce((acc, s) => {
      acc[s.scanType] = (acc[s.scanType] ?? 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const byStatus = allScans.reduce((acc, s) => {
    ...
    return {
      data: {
        summary: {
          ...
          criticalFindings,
          failedPolicies,
          coveragePercent: assets.length > 0
            ? Math.round((scannedAssetIds.size / assets.length) * 100)
            : 0,
  ...
  getDrift: protectedProcedure.query(async ({ ctx }) => {
    const assets = await prisma.aIAsset.findMany({
      ...
      select: {
        ...
        euRiskLevel: true, updatedAt: true, createdAt: true,
        scanRecords: {
          orderBy: { startedAt: "desc" },
          take: 2,
          select: {
            id: true, scanType: true, status: true,
            findingsCount: true, criticalFindings: true,
            policyPassed: true, startedAt: true
          }
        }
      }
    });
    ...
    const driftSignals = assets.map(asset => {
      ...
      const findingsDelta = lastScan && prevScan
        ? (lastScan.findingsCount ?? 0) - (prevScan.findingsCount ?? 0)
        : null;
      ...
      if (asset.euRiskLevel === "HIGH" && (!lastScan || (daysSinceScan ?? 999) > 30)) signals.push("HIGH_RISK_STALE");

      const severity =
        signals.includes("POLICY_FAILED") || signals.includes("HIGH_RISK_STALE") ? "HIGH" :
        signals.includes("FINDINGS_INCREASED") || signals.includes("SCAN_OVERDUE") ? "MEDIUM" :
        signals.includes("NEVER_SCANNED") ? "MEDIUM" : "LOW";

      return {
        assetId: asset.id, assetName: asset.name, assetType: asset.assetType,
        euRiskLevel: asset.euRiskLevel, status: asset.status,
        daysSinceUpdate, daysSinceScan,
        lastScanStatus: lastScan?.status ?? null,
        lastPolicyPassed: lastScan?.policyPassed ?? null,
        findingsDelta, signals, severity
      };
    });
    ...
    return {
      data: {
        ...
        driftSignals: driftSignals.sort((a, b) =>
  ...
  getAlerts: protectedProcedure.query(async ({ ctx }) => {
    const [scans, assets, securityEvents] = await Promise.all([
      ...
      prisma.aIAsset.findMany({
        where: { orgId: ctx.orgId, deletedAt: null },
        select: {
          id: true, name: true, euRiskLevel: true, status: true, updatedAt: true,
    ...
    const alerts: {
      ...
      assetId?: string; assetName?: string; createdAt: Date;
    }[] = [];
    ...
    for (const asset of assets.filter(a =>
      a.euRiskLevel === "HIGH" && a.accountabilityAssignments.length === 0
    )) {
      alerts.push({
